            ...
            if (_.isArray(json)) {
                ...
                json.forEach(function(obj) {
                    ...
                    map(obj, function(err, result) {
                        ...
                        if (values.length === json.length) return cb(null, values);
                    });
                });
            } else {
                map(json, function(err, values) {
...
module.exports = {
    ...
    constructUri: function(connection, action, options, context) {
        ...
        _.keys(action.urlParameters).forEach(function(key) {
            iUrlParameters[key] = self.interpolate(action.urlParameters[key], context);
        });

        var iOptions = {};
        _.keys(options).forEach(function(key) {
            iOptions[key] = self.interpolate(options[key], context);
        });
    ...
    constructBody: function(connection, action, context) {
        var self = this;
        ...
        return ''; //TODO: Return the current 'object'?
    },
    makeRequest: function(connection, model, action, options, values, context, cb) {
        ...
        var params = {
            ...
            method: action.verb,
            body: self.constructBody(connection, action)
        };
