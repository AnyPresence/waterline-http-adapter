...
function JsonPathFinder(payload) {
    return {
        get: function(p) {
            return jsonPath.eval(payload, p);
        }
    };
}

// TODO: XML support
function XmlPathFinder(payload) {
    return {};
}

module.exports = { 
    mapFields: function(payload, model, action) {
        var finder = new JsonPathFinder(payload);
        var newModel = {};
        ...
        _.keys(attributes).forEach(function(key) {
            if (_.has(mapping, key)) {
                var val = finder.get(mapping[key]);

                // jsonpath library always returns arrays, if the attribute is not
                // an array, take the first value of the array
                if (_.isArray(val) && attributes[key].type !== 'array') {
                    val = val[0];
                }
